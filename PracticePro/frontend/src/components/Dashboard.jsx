...
const Dashboard = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://localhost:8000/api/content/generate-content", { prompt });
      ...
      setGeneratedData([...generatedData, { heading, subheading, date, articles }]);
    } catch (error) {
  ...
  return (
    <div>
      <div className="flex">
        ...
        <button onClick={handleSubmit} className="w-30 h-10 bg-blue-700 py-2 mt-2  mr-2 rounded-lg text-white px-3">
          generate
        </button>
      </div>
